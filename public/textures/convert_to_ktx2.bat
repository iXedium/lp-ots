...
echo =============================================
echo  UASTC Quality: %UASTC_QUALITY%
echo  UASTC RDO:     %UASTC_RDO%
...
for %%F in ("%~dp0*.png") do (
    ...
    if "!TYPE!"=="AO" (
        ...
        echo   Type: Normal ^(UASTC, linear, normalmap^)
        toktx --t2 --encode uastc --uastc_quality %UASTC_QUALITY% --uastc_rdo_l 0.25 --zcmp %ZSTD_LEVEL% --genmipmap --assign_oetf linear --normal_mode --normalize "!OUTPUT!" "!INPUT!"
    ) else if "!TYPE!"=="DATA" (
